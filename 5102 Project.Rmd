...
```{r, results=TRUE}
OLS_anderson <- ad.test(OLS_residuals)
...
Many of these predictor variables are linear combinations of others. They provide no useful information; their presence increase variance in coefficient estimation and reduce statistical power in proving the coefficients are useful.
...
```{r, results=TRUE}
Multiple_anderson <- ad.test(Multiple_residuals)
...
### Heteroskedasticity test
```{r, results=TRUE}
Multiple_bp <- bptest(Multiple_model)
...
Again, none of the Normality test passed.
...
```{r, results=TRUE}
Multiple_log_anderson <- ad.test(Multiple_log_residuals)
...
### Heteroskedasticity test
```{r, results=TRUE}
Multiple_log_bp <- bptest(Multiple_log)
...
Two out of three normality tests passed; studentized Breusch-Pagan test also passed. 
...
```{r, results=TRUE}
Lasso_multiple_anderson <- ad.test(Lasso_multiple_residuals)
...
### Heteroskedasticity test
```{r, results=TRUE}
Lasso_multiple_bp <- bptest(Lasso_multiple_model)
...
Given the vast number of predictor variables, one must filter useful variables from unuseful ones. Including variables that provide no extra information in predicting the response will inflate variance; estimates of coefficients will be unstable in regards to other predictor variables, and statistical power will suffer due to inflated standard error. 
...
Let $M_0$ denote the null model which contains no predictors, this model simply predicts the sample mean of each observation
  ...
  - Select the single best model among $M_0$, $M_1$,...,$M_n$ mean squared error and adjusted $R^2$.
...
For every $k_i$ (from 1 to `r ncol(world_bank_imputed)`), we pick out the subset with smallest test MSE, then store it in a dataframe. We also pick out the subset with highest test adjusted $R^2$, and store them in another dataframe. 
...
The graph on the left illustrates the mean square error (mse) for each model among each $k$ number of predictor variables. The red line connects models with the lowest mse in each $k$. The graph on the right shows the same for adjuated $R^2$. 

According to the graphs above, the more predictor variables included in the model, the lower the mse and higher the adjusted $R^2$. How ever, both red lines level off at 6 predictors. This suggests the difference in model performance between 6 and 11 predictor variables are miniscure. In the spirit of parsimoniousness, one should opt for a simpler model with fewer variables. This minimises colinearity and variance in estimates of predictor coefficients.   
...
The relation between `U5.Mortality` and the predictor variables are highly non-linear. As mention a few sections back, log transformation on both the response and predictor can improve linearity as well as maintaining linear regression assumptions.
...
plt.suptitle("U5 Mortality v.s. Predictor (transformed)")
plt.tight_layout()
fig.subplots_adjust(top=0.9)
plt.show()
```
...
```{r, results=TRUE}
Best_sub_anderson <- ad.test(Best_sub_residuals)
...
### Heteroskedasticity test
```{r, results=TRUE}
Best_sub_bp <- bptest(Best_sub_model)
