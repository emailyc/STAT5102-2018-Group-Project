...
def regression_graph(true_values, fitted_values, residuals):
  ...
  ax2.scatter(fitted_values, residuals, s = 1)
...
mcor<-round(cor(cor(world_bank[mortality_names])),2)
upper<-mcor
...
We see that everything is linearly dependant; they are essentially the same measure. Highly correlated predictors can lead to instability in our estimator, as well as increased variance. 
...
Both Shapiro–Wilk (Shapiro, & Wilk, 1965) and Anderson-Darling (Anderson, & Darling, 1952) tests tests the null hypothesis that a statistical sample $x_1$, ..., $x_n$ came from a normally distributed population. Here, these tests are used to test for normality of the model residuals. Normality tests tend to be not very statistically powerful, hence more than one tests are used.
...
The p-value is `r Lasso_f_p`, we reject H0 and conclude that these independent variables have an overall effect on Y. 
...
The regression plot gives a few insights:
...
The p-value is `r Best_sub_f_p`, we reject $H_0$ and conclude that these independent variables have an overall effect on Y. 
...
```
According to validation, the Bestsubset model yields RMSE:`Best_Subset_Validation_Result[1]` and $R^2$: `Best_Subset_Validation_Result[2]`; Lasso model yields RMSE:`Lasso_Validation_Result[1]` and $R^2$: `Lasso_Validation_Result[2]`
There are less error in Bestsubset's prediction, as well as more explained variance. Hence Bestsubset model is the model whcih performs best. 
