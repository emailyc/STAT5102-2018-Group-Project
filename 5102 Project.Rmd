...
Multiple_log_model <- lm(data = Multiple_train, I(log(U5.Mortality)) ~ .)
summary(Multiple_log_model)
Multiple_log_fitted_values <- predict(Multiple_log_model, newdata = Multiple_train)
Multiple_log_residuals <- resid(Multiple_log_model)
```

#### The test RMSE and $R^2$ are as follow
```{r, results=TRUE}
postResample(pred = predict.lm(Multiple_log_model, Multiple_test), obs = log(Multiple_test$U5.Mortality))
```
...
#### Normality Test
```{r, results=TRUE}
shapiro_test(Multiple_log_residuals)
```

```{r, results=TRUE}
anderson_test(Multiple_log_residuals) 
```

#### Heteroskedasticity test
```{r, results=TRUE}
breusch_pagan_test(Multiple_log_model)
```
...
Lasso_f_test <- anova(lm(data = Multiple_train, log(U5.Mortality)~1), Lasso_multiple_model)
Lasso_f_p <- Lasso_f_test$`Pr(>F)`
```
...
Best_sub_f_p <- Best_sub_f_test$`Pr(>F)`
```
...
$$
\begin{align*}
\mathrm{\hat{Price}} = `r Best_sub_full_coef[1]` 
