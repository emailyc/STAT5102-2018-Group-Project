...
Lasso_multiple_model <- lm(data = world_bank_imputed[c(lasso_terms, "U5.Mortality")], log(U5.Mortality) ~ .)
summary(Lasso_multiple_model)$call
Lasso_multiple_fitted_values <- predict(Lasso_multiple_model, newdata = world_bank_imputed)
Lasso_multiple_residuals <- resid(Lasso_multiple_model)
Lasso_full_coef <- paste(round(coef(Lasso_multiple_model), 2))
Lasso_f_test <- anova(lm(data = world_bank_imputed, log(U5.Mortality)~1), Lasso_multiple_model)
...
```{python}
regression_graph(r.world_bank_imputed["U5.Mortality"].apply(np.log), r.Lasso_multiple_fitted_values, r.Lasso_multiple_residuals)
```
